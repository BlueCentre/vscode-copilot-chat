...
class ChatAgents implements IDisposable {
	...
	private async personalizeWelcomeMessage() {
		try {
			...
			const prompts: { title: string; prompt: string; detail: string }[] = [
				...
				{ title: '💡 Add tests for a function in the open file', prompt: 'Write unit tests for the main exported function in the active file. Use the existing test framework already configured in this repo.', detail: 'Increase coverage' }
			];
			let mdValue = `### Hi, ${firstName}!\n`;
			mdValue += `Signed in as **${user.login}**. Here are some quick starters:`;
			mdValue += `\n\n` + prompts.map(p => `**${p.title}**  [${p.detail}]  \\\n👉 [Try it](command:workbench.action.chat.open?${encodePrompt(p.prompt)})`).join('\n\n');
	...
	private registerReplayAgent() { (this.createAgent('chatReplay', Intent.ChatReplay) as any).iconPath = new vscode.ThemeIcon('copilot'); }
	...
	private registerDefaultAgent() {
		...
		const defaultAgent = this.createAgent(defaultAgentName, intentGetter) as any;
...
Learn more about [GitHub Copilot](https://docs.github.com/copilot/using-github-copilot/getting-started-with-github-copilot?tool=vscode&utm_source=editor&utm_medium=chat-panel&utm_campaign=2024q3-em-MSFT-getstarted) in [Visual Studio Code](https://code.visualstudio.com/docs/copilot/overview). Or explore the [Copilot walkthrough](command:swe.agent.open.walkthrough).`,
		...
		const md = new vscode.MarkdownString(helpPostfix); md.isTrusted = { enabledCommands: ['inlineChat.start', 'swe.agent.open.walkthrough'] }; defaultAgent.helpTextPostfix = md;
		defaultAgent.additionalWelcomeMessage = this.additionalWelcomeMessage; defaultAgent.titleProvider = this.instantiationService.createInstance(ChatTitleProvider); defaultAgent.summarizer = this.instantiationService.createInstance(ChatSummarizerProvider);
	}
