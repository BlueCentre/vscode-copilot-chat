...
export const CreateProjectCommand = 'swe.agent.createProject';
export const CreateFileCommand = 'swe.agent.createFile';
export const OpenFileCommand = 'swe.agent.openFile';
...
class NewWorkspaceResponseProcessor {
	...
	async processResponse(context: IResponseProcessorContext, inputStream: AsyncIterable<IResponsePart>, outputStream: vscode.ChatResponseStream, token: vscode.CancellationToken): Promise<void> {
		...
		if (turn.id &&
			...
			const markdownString = new MarkdownString(l10n.t(`Hint: You can [regenerate this project without using this sample](command:workbench.action.chat.open?{0}) or use this [setting](command:workbench.action.openSettings?%5B%22swe.agent.chat.useProjectTemplates%22%5D) to configure the behavior.`, query));
