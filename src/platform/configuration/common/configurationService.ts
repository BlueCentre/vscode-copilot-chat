...
export interface BaseConfig<T> {
	/**
	 * Key as it appears in settings.json minus the "swe.agent." prefix.
	...
	/**
	 * The fully qualified id, e.g. "swe.agent.advanced.debug.overrideProxyUrl".
	 * Use this with `affectsConfiguration` from the ConfigurationChangeEvent
	 */
	readonly fullyQualifiedId: string;

	/**
	 * The `X` in `swe.agent.advanced.X` settings.
	 */
...
function toBaseConfig<T>(key: string, defaultValue: T | DefaultValueWithTeamValue<T> | DefaultValueWithTeamAndInternalValue<T>, options: ConfigOptions | undefined): BaseConfig<T> {
	...
	const advancedSubKey = fullyQualifiedId.startsWith('swe.agent.advanced.') ? fullyQualifiedId.substring('swe.agent.advanced.'.length) : undefined;
	return { id: key, isPublic, fullyQualifiedId, advancedSubKey, defaultValue, options };
}

class ConfigRegistry {
	/**
	 * A map of all registered configs, keyed by their full id, eg `swe.agent.advanced.debug.overrideProxyUrl`.
	 */
 ...
 * ```
 *     config.swe.agent.${key}
 *
 * e.g.
 *     config.swe.agent.chat.advanced.inlineEdits.internalRollout
 * ```
 */
